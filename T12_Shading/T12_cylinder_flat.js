...
var numDivision = 36; 
var shaderProgram;
var num_vertex = (numDivision) * 2   * 2; 
var opacity1 = 1.0; 
var opacity2 = 1.0; 
...
function makeCylinder(r,h,n)
{
	...
	{
		// Vertex Position
		viewVertex[i*48 +  0] = viewVertex[i*48 +  12] = Math.cos(Math.PI / 180.0 * (360.0 / n ) * i); 
		viewVertex[i*48 +  1] = -h/2.0;   
		viewVertex[i*48 + 13] =  h/2.0; 
		viewVertex[i*48 +  2] = viewVertex[i*48 + 14] = Math.sin(Math.PI / 180.0 * (360.0 / n ) * i); 
		// Color Attrub all light grey; 
		viewVertex[i*48 +  3] = viewVertex[i*48 + 15] = 0.75; 
		viewVertex[i*48 +  4] = viewVertex[i*48 + 16] = 0.75; 
		viewVertex[i*48 +  5] = viewVertex[i*48 + 17] = 0.75; 
		viewVertex[i*48 +  6] = opacity1;
		viewVertex[i*48 + 18] = opacity2;
		// Texture UV
		viewVertex[i*48 +  7] = viewVertex[i*48 + 19] = i / n; 
		viewVertex[i*48 +  8] = 1.0;
		viewVertex[i*48 + 20] = 0.0;
		// Normal 
		viewVertex[i*48 +  9] = viewVertex[i*48 + 21] = Math.cos(Math.PI / 180.0 * (360.0 / n ) * i); 
		viewVertex[i*48 + 10] = viewVertex[i*48 + 22] = 0.0;
		viewVertex[i*48 + 11] = viewVertex[i*48 + 23] = Math.sin(Math.PI / 180.0 * (360.0 / n ) * i); 
		
		// Vertex Position
		viewVertex[i*48+24 +  0] = viewVertex[i*48+24 +  12] = Math.cos(Math.PI / 180.0 * (360.0 / n ) * (i+1)); 
		viewVertex[i*48+24 +  1] = -h/2.0; 
		viewVertex[i*48+24 + 13] =  h/2.0; 
		viewVertex[i*48+24 +  2] = viewVertex[i*48+24 + 14] = Math.sin(Math.PI / 180.0 * (360.0 / n ) * (i+1)); 
		// Color Attrub all light grey; 
		viewVertex[i*48+24 +  3] = viewVertex[i*48+24 + 15] = 0.75; 
		viewVertex[i*48+24 +  4] = viewVertex[i*48+24 + 16] = 0.75; 
		viewVertex[i*48+24 +  5] = viewVertex[i*48+24 + 17] = 0.75; 
		viewVertex[i*48+24 +  6] = opacity1;
		viewVertex[i*48+24 + 18] = opacity2;
		// Texture UV
		viewVertex[i*48+24 +  7] = viewVertex[i*48+24 + 19] = i / n; 
		viewVertex[i*48+24 +  8] = 1.0;
		viewVertex[i*48+24 + 20] = 0.0;
		// Normal 
		viewVertex[i*48+24 +  9] = viewVertex[i*48+24 + 21] = Math.cos(Math.PI / 180.0 * (360.0 / n ) * i); 
		viewVertex[i*48+24 + 10] = viewVertex[i*48+24 + 22] = 0.0;
		viewVertex[i*48+24 + 11] = viewVertex[i*48+24 + 23] = Math.sin(Math.PI / 180.0 * (360.0 / n ) * i); 
	}
	return(viewVertex); 
}
